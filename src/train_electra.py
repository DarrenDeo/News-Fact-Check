...
# "indobenchmark/indoelectra-base-p1-discriminator"
MAX_LEN = 256
BATCH_SIZE = 8 # Sesuaikan berdasarkan VRAM GPU Anda
EPOCHS = 3
LEARNING_RATE = 2e-5 # Bisa juga mencoba 3e-5 atau 5e-5 untuk ELECTRA
OUTPUT_DIR = "../models/electra" # Direktori penyimpanan untuk model ELECTRA
...
class NewsDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        encoding = self.tokenizer.encode_plus(
            ...
            return_token_type_ids=False, # ELECTRA juga tidak selalu memerlukan token_type_ids untuk klasifikasi
            padding='max_length',
            truncation=True,
...
def main():
    print("Memulai pelatihan model ELECTRA...")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Menggunakan device: {device}")

    print(f"Memuat tokenizer dari: {MODEL_NAME}")
    # Gunakan ElectraTokenizerFast
    ...
    # Gunakan ElectraForSequenceClassification
