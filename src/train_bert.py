...
from tqdm.auto import tqdm # Menggunakan tqdm.auto untuk kompatibilitas notebook/script
import os
import numpy as np
...
class NewsDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        encoding = self.tokenizer.encode_plus(
            ...
            return_token_type_ids=False, # Tidak diperlukan untuk klasifikasi sekuens tunggal
            padding='max_length',
            truncation=True, # Memotong sekuens yang lebih panjang dari max_len
            return_attention_mask=True, # Mengembalikan attention mask
            return_tensors='pt',  # Mengembalikan PyTorch tensors
        )

        return {
            'text': text, # Menyertakan teks asli untuk debugging (opsional)
...
def load_data(file_path):
    try:
        ...
        df.dropna(subset=['text', 'label'], inplace=True) # Hapus baris dengan teks atau label kosong
...
def train_epoch(model, data_loader, optimizer, device, scheduler, n_examples):
    model = model.train() # Set model ke mode training
    total_train_loss = 0
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        optimizer.zero_grad() # Kosongkan gradien dari batch sebelumnya

        outputs = model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            labels=labels # Memberikan labels agar loss dihitung oleh model
        )
        ...
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0) # Mencegah exploding gradients
        optimizer.step() # Update bobot
        scheduler.step() # Update learning rate (jika menggunakan scheduler per step)
        
        # Update progress bar description
        if (batch_idx + 1) % 10 == 0: # Update setiap 10 batch
            current_loss = total_train_loss / (batch_idx + 1)
            # Hati-hati dengan pembagian dengan ukuran batch terakhir yang mungkin lebih kecil
            num_processed_samples = (batch_idx + 1) * data_loader.batch_size
            if batch_idx + 1 == len(data_loader): # Jika ini batch terakhir
            ...
            current_acc = min(current_acc, 1.0) # Pastikan tidak lebih dari 1.0
...
def eval_model(model, data_loader, device, n_examples):
    model = model.eval() # Set model ke mode evaluasi
    total_eval_loss = 0
    ...
    # Hitung presisi, recall, F1 secara keseluruhan (weighted)
...
def main():
    ...
    if not train_texts or not val_texts:
        ...
        print(f"  Pastikan file 'train.csv' dan 'val.csv' ada di '{PROCESSED_DATA_DIR}' dan berisi data.")
        return
    ...
    for epoch in range(EPOCHS):
        ...
        # Hitung jumlah sampel aktual untuk train_epoch
